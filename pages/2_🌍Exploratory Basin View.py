...
SELECT * FROM climate_resilience.mv_major_basin_waterpoints WHERE basin_id={basin_selection_id}''')
# Executing query 
...
### OBTAINING THE PERCENTAGE OF HIGHLY VULNERABLE AREA IN THE SELECTED BASIN

# Creating a title for the page
st.write(f"# {basin_selection} Basin Descriptive Statistics")
...
# Filtering for the selected basin
df_merged_filter = df_merged_filter[df_merged_filter['basin_id']==basin_selection_id]

# Computing the area of the wards in the filtered dataframe
df_merged_filter['area'] = df_merged_filter['geometry'].area

# Creating a groupby 
high_risk_clusters_basin = df_merged_filter.groupby(['basin_id'])['area'].agg(['sum','count']).reset_index()
high_risk_clusters_basin.columns = ['basin_id','total_area_meters_2','vulnerable_ward_count']
high_risk_clusters_basin['total_area_km2'] = high_risk_clusters_basin['total_area_meters_2'] / 1000000
...
normalized_vulnerable_percentage = (high_risk_clusters_basin['total_area_km2'].item()/population_stats['area_sqkm'].item())*100 
